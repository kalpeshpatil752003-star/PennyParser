...
def extract_financial_tables(file_path: str) -> list[dict]:
    """Returns [{ "page_number": int, "statement_type": str, "rows": [[cell,...], ...] }, ...]"""
    results = []
    with pdfplumber.open(file_path) as pdf:
        for i, page in enumerate(pdf.pages):
            ...
            if not statement_type:
                continue
            print(f"[classify] page {i + 1} classified as {statement_type}")
...
def extract_financial_tables(file_path: str) -> list[dict]:
    ...
    with pdfplumber.open(file_path) as pdf:
        for i, page in enumerate(pdf.pages):
            ...
            print(f"[debug] page {i+1}: found {len(tables)} raw tables")
            for t_idx, table in enumerate(tables):
                print(f"[debug] page {i+1} table {t_idx}: {len(table)} rows")
